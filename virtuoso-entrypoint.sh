...
#
#  Copyright (C) 2018-2022 OpenLink Software
#
...
export SETTINGS_DIR="/opt/virtuoso-opensource/settings"
export DATABASE_DIR="/opt/virtuoso-opensource/database"
#
#  Default programs
#
...
generate_initial_password() {
	...
	#
	echo "$DBA_PASSWORD" > "${SETTINGS_DIR}/dba_password"
	echo "$DAV_PASSWORD" > "${SETTINGS_DIR}/dav_password"
}
...
initialize_virtuoso_directory()
{
	...
	then
		...
		then
			cp "$VIRTUOSO_INI_FILE" "${DATABASE_DIR}/virtuoso.ini"
		else
			cp "$VIRTUOSO_HOME"/installer/virtuoso.ini.sample "${DATABASE_DIR}/virtuoso.ini"
		fi
